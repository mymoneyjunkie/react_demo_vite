...
const Card = ({ lang, isPosts, path, rounded, isHome, isDataLoaded, isImageLoaded, imgUpload, className = '', trending, ...rest}) => {
	...
	return (
		...
		>
			<div
				className={`${rounded ? 'rounded-full' : 'rounded-sm'} ${trending ? 'w-50' : 'w-full'} h-48 lg:w-full bg-gradient-to-r`}
			...
			<div className={`font-medium ${isHome && isDataLoaded ? 'bg-black/80 p-3 h-20' : ''} ${isPosts && isDataLoaded ? 'bg-black/80 pt-3 h-30 flex flex-col justify-between' : ''} ${!isDataLoaded ? 'animate-pulse' : ''}`}>
				...
				{isPosts && (
					<>
						...
						<div className={`mt-2 p-2 text-sm flex justify-center items-center bg-black/90 text-black dark:text-white ${isHome || isPosts && 'text-white text-center'} ${!isDataLoaded ? 'bg-gray-400' : ''}`}>
